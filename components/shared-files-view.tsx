...
export default function SharedFilesView() {
  ...
  const fetchSharedFiles = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        console.error("Failed to load shared files:", errorData)
        toast.error("Failed to load shared files")
      }
    } catch (error) {
      console.error("Error fetching shared files:", error)
      setError("Failed to load files shared with you. Please try again later.")
      toast.error("Something went wrong while loading shared files")
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const filteredFiles = files.filter((file) => {
    ...
    const userName = file.user?.name?.toLowerCase() || file.user?.email?.toLowerCase() || ""
    const searchLower = searchQuery.toLowerCase()
    return fileName.includes(searchLower) || userName.includes(searchLower)
  })
        ...
        <main className="flex-1 overflow-auto p-6">
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center h-64 text-center bg-card rounded-lg border p-8 fade-in">
              ...
              <Button onClick={fetchSharedFiles} className="mt-4">
                Try Again
              </Button>
            </div>
          ) : (
