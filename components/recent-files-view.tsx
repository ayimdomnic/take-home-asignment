  ...
  const fetchRecentFiles = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        console.error("Failed to load recent files:", errorData)
        toast.error("Failed to load recent files")
      }
    } catch (error) {
      console.error("Error fetching recent files:", error)
      setError("Failed to load your recent files. Please try again later.")
      toast.error("Something went wrong while loading your recent files")
    } finally {
      setIsLoading(false)
    }
  }
        ...
        <main className="flex-1 overflow-auto p-6">
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center h-64 text-center bg-card rounded-lg border p-8 fade-in">
              ...
              <Button onClick={fetchRecentFiles} className="mt-4">
                Try Again
              </Button>
            </div>
          ) : (
