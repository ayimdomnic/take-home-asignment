...
export function Sidebar({ currentFolderId, isOpen, onToggle }: SidebarProps) {
  ...
  return (
    <div
      className={cn(
        "hidden md:block border-r border-border/50 bg-card transition-all duration-300",
        isOpen ? "w-64" : "w-0",
      )}
    >
      {isOpen && (
        <div className="h-full flex flex-col">
          ...
          <div className="p-4">
            <Button className="w-full justify-center mb-6 bg-primary hover:bg-primary/90 shadow-sm">
              <Plus className="mr-2 h-4 w-4" />
              New
            </Button>
          ...
          <div className="mt-auto p-4 border-t">
            <div className="mb-2">
              ...
              <Progress value={storagePercent} className="h-1.5 bg-muted/40" />
              <div className="mt-1 text-xs text-right">
                <span className="text-muted-foreground">{(storageTotal - storageUsed).toFixed(1)} GB free</span>
              </div>
            </div>
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
      ...
      {!isOpen && (
        <Button variant="ghost" size="icon" onClick={onToggle} className="absolute top-4 left-4 z-10">
