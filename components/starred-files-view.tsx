...
export default function StarredFilesView() {
  ...
  const fetchStarredFiles = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        console.error("Failed to load starred files:", errorData)
        toast.error("Failed to load starred files")
      }
    } catch (error) {
      console.error("Error fetching starred files:", error)
      setError("Failed to load your starred files. Please try again later.")
      toast.error("Something went wrong while loading your starred files")
    } finally {
      setIsLoading(false)
    }
  }
        ...
        <main className="flex-1 overflow-auto p-6">
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center h-64 text-center bg-card rounded-lg border p-8 fade-in">
              ...
              <Button onClick={fetchStarredFiles} className="mt-4">
                Try Again
              </Button>
            </div>
          ) : (
